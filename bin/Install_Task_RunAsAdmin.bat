...
:: Set paths
set "EXE_PATH=%scriptDir%\Unified-WinProgManager.exe"
...
echo Get-CimInstance Win32_Process -Filter "Name = 'Unified-WinProgManager.exe'" ^| >> "%PS1_PATH%"
echo     ForEach-Object { Stop-Process -Id $_.ProcessId -Force } >> "%PS1_PATH%"
echo. >> "%PS1_PATH%"
echo # Launch new instance (run hidden) >> "%PS1_PATH%"
echo Start-Process -FilePath "%EXE_PATH%" -WindowStyle Hidden >> "%PS1_PATH%"
